...
**FN** se incorpora como familia analítica construida a partir de **11 diputados identificados por `deputy_id`** (electos 2017 bajo bandera FN/RN, sus suplentes y Emmanuelle Ménard), hasta ahora diluidos en `NI`. Tras separarlos, **`NI` queda como familia residual** (8 diputados no inscritos heterogéneos) y **ya no debe interpretarse como proxy de FN/RN**. Los grupos pequeños (NI 8, EDS 9, FN 11, LT 13, LFI/GDR ~17) tienen pocos diputados, así que su tasa de Pour por scrutin descansa en pocas personas; el filtro de ≥3 diputados expresando voto por scrutin (`MIN_EXPRESSED=3`) y la ponderación por nº de votos lo tienen en cuenta.
