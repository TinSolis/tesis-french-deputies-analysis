...
> **Cambios recientes (estado del repo).** (1) La **tesis ya está escrita y aprobada**; el estado real del documento final vive en `tesis_escrita_context.md`. (2) En el **cuerpo de la tesis** los **dominios MARPOR se escriben en español** (Relaciones exteriores, Libertad y democracia, Sistema político, Economía, Bienestar y calidad de vida, Tejido social, Grupos sociales); en este documento de datos se conservan los nombres en inglés del esquema como referencia técnica. (3) Los **README de `french_deputies/`** se reescribieron para mayor claridad y se corrigieron rutas: la carpeta real es **`french_deputies/`** (no `francia_deputies/`) y la de Twitter es **`twitter_zeeschuimer/`** (no `zeeschuimer/`). (4) El documento de propuesta ahora vive en **`memoria/propuesta/`**. Estas correcciones resuelven parte del punto §11.9.
...
**El canal define la agenda (Análisis 1):** manifiestos = *programa* (domina Welfare & QoL); tweets y hemiciclo = *meta-política* (domina Political System, 30–42 %). El efecto canal pesa ~13–16 pp para todos. Descontado, **FN** es el **mayor reorganizador estimable** (shift específico 17,5; Twitter funciona como *outlier* que lo lleva a Autoridad Política, pero su núcleo 601/605 persiste en los 3 canales), seguido de **LFI** (12,1); **LREM** la mantiene (7,4; persistencia 0,412). Pero la **firma distintiva persiste** (issue ownership: ecologistas→ambiente, PCF→trabajadores, **FN→identidad nacional y ley y orden** —incorporada como familia analítica propia, separada del residuo heterogéneo NI, que ya no es proxy de FN/RN).

**El voto tiene dos capas (Análisis 2):** el eje se **invierte** leyes↔enmiendas (LREM 99,1 %→15,7 %; LFI 11,5 %→83,7 %; FN 27,1 %→73,4 %). En leyes el binario mayoría/oposición ya explica R²=0,47; en enmiendas todo explica menos (0,33) y el tema como dominio dominante casi no añade varianza (+1 pp). Pero aparece un **clivaje cultural robusto** en el soporte relativo de enmiendas: la izquierda apoya *de menos* *Fabric of Society* (PS −21,9, GDR-PCF −17,5, LFI −15,9) y la derecha *de menos* *Freedom & Democracy* (**LR −19,1 y FN −19,2**; antes esa señal aparecía atribuida a NI porque lo incluía) — signos estables en ~100 % del bootstrap por scrutin, repartidos en >50 leyes. FN suma además **Welfare & QoL +14,0** y no es distintivo en *Fabric of Society*. La **economía polariza menos**. **Cohesión Rice** valida el agrupamiento (LFI 0,998; **FN 0,955** pese a su base parlamentaria pequeña; el **NI residual 0,495**, el menos cohesionado, y LT ~0,74).

**Brecha declarado–revelado (Análisis 3):** la coherencia por partido es **débil e inestable** (IC ≈ [−1,+1]; pooled **0,08–0,16**) → **no rankeable** ni leíble como juicio normativo; el aporte es **tipológico**. Patrones localizados: banderas afirmativas (LFI↔libertades +6,9; PS↔bienestar; centro gubernamental↔economía), **coherencia negativa** de la izquierda en el eje cultural (el patrón más limpio), y **énfasis no respaldado** (LR↔cultura; y el caso extremo **FN**, que declara una firma nacional-securitaria persistente pero cuyo voto en *Fabric of Society* no es distintivo, revela apoyo a Welfare no enfatizado y se distancia de Libertades —un gap entre capas observables, no incoherencia moral). Ningún canal anticipa el voto mejor que otro una vez igualada la cobertura.
...
- **Correlaciones por partido como secundarias:** con n=6 dominios y 7–11 partidos, los coeficientes son ruidosos; se reportan con IC para mostrar fragilidad, no para concluir → el peso recae en la **tipología** y los **signos por celda**.
- **FN/RN como familia analítica:** los diputados FN/RN figuran como `NI` en los datos (no formaron grupo); se reagrupan como **`FN`** vía override por **11 `deputy_id`** (`party_analysis/common/families.py`), **sin convertir todo `NI`** (el resto queda residual, **no proxy de FN/RN**). Caveats: base parlamentaria pequeña; Houplain y Évrard sin tweets; Évrard dejó el FN en nov-2017 (atribución posterior con cautela); `MIN_EXPRESSED=3` limita sus scrutins; soporte relativo negativo ≠ rechazo absoluto; sin causalidad.
...
- **Heterogeneidad de "partido" entre canales:** manifiesto = texto oficial del partido; tweets/hemiciclo = texto de diputados individuales; voto = grupo parlamentario. El manifiesto se indexa por partido electoral y el voto por grupo → **7 partidos** (incluida FN, separada del residuo NI) casan limpio en el Análisis 3. Desfase temporal/organizacional (campaña 2017 vs. legislatura).
...
- **Muestras chicas:** PCF (39) y PS (79) en manifiestos → firmas indicativas; **LR y UDI idénticos** en manifiestos (posible duplicación). En CHES n=8; en Análisis 3, n=7 (manifiesto) a 11 (tweets/hemiciclo) partidos.
